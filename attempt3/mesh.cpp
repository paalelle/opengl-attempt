...
void Texture::SetGLInfo(){
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);	//缩小时纹理采样使用线性过滤模式
...
void Mesh::Draw(Shader *dshader){
	...
	for(unsigned i = 0; i < textures.size(); i++){
		...
		//命名规则:texture + 纹理类别缩写 + 纹理序号
		//设置着色器中每个sampler对应的纹理单元
		dshader->SendUniform_Int(i, ("texture" + texTypeName + texNumber).c_str());
