	...
	glm::mat4 totrotate =
		glm::rotate(
			glm::rotate(
				...
				pitch, glm::vec3(1.0, 0.0, 0.0)),
			roll, glm::vec3(0.0, 0.0, 1.0));
...
void ModelLinker::DrawAll(glm::mat4 mview, glm::mat4 mproj){
	...
	mshader->SendUniform_Mat4(glm::value_ptr(mproj), 1, "umatrices.proj");
	for(size_t i = 0; i < modelMatrices.size(); i++){

		normalMatrix = glm::mat3(transpose(inverse(modelMatrices[i])));
		mvMatrix = mview * modelMatrices[i];

		mshader->SendUniform_Mat4(glm::value_ptr(modelMatrices[i]), 1, "umatrices.model");
		mshader->SendUniform_Mat4(glm::value_ptr(mvMatrix), 1, "umatrices.mv");
		mshader->SendUniform_Mat3(glm::value_ptr(normalMatrix), 1, "umatrices.normal");
